...
void main() {
  group('expandCidr', () {
    test('/24 expands to 254 host addresses (excludes net + broadcast)', () {
      ...
      expect(hosts.contains('10.0.0.0'), isFalse,
          reason: 'network address must be excluded');
      expect(hosts.contains('10.0.0.255'), isFalse,
    ...
    test('rejects non-CIDR string', () {
      expect(() => expandCidr('not-a-cidr').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects prefix > 32', () {
      expect(() => expandCidr('10.0.0.1/33').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects negative prefix', () {
      expect(() => expandCidr('10.0.0.1/-1').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects non-numeric prefix', () {
      expect(() => expandCidr('10.0.0.1/abc').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects too-few octets', () {
      expect(() => expandCidr('10.0.0/24').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects too-many octets', () {
      expect(() => expandCidr('10.0.0.1.2/24').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects empty octet', () {
      expect(() => expandCidr('10..0.1/24').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects octet > 255', () {
      expect(() => expandCidr('10.0.0.256/24').toList(),
          throwsA(isA<FormatException>()));
    });

    test('rejects missing prefix separator', () {
      expect(() => expandCidr('10.0.0.1').toList(),
          throwsA(isA<FormatException>()));
    });
