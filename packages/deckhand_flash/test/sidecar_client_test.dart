...
void main() {
  group('SidecarClient response routing', () {
    test('successful response resolves the pending future with result',
        () async {
    ...
    test('error response rejects with SidecarError carrying code + message',
        () async {
      final client = SidecarClient(binaryPath: '/does/not/exist');
      final completer = Completer<Map<String, dynamic>>();
    ...
    test('subscribeToOperation receives only notifications for its op id',
        () async {
      final client = SidecarClient(binaryPath: '/does/not/exist');
      final opNotes = <SidecarNotification>[];
      final sub = client.subscribeToOperation('op-A').listen(opNotes.add);

      client.handleLineForTesting(jsonEncode({
        'jsonrpc': '2.0',
        'method': 'progress',
    ...
    test('out-of-order responses route to the correct pending caller',
        () async {
      final client = SidecarClient(binaryPath: '/does/not/exist');
    ...
    test('notification without operation_id still dispatches globally',
        () async {
      final client = SidecarClient(binaryPath: '/does/not/exist');
      final received = <SidecarNotification>[];
      final sub = client.notifications.listen(received.add);
    ...
    test('numeric id from sidecar still correlates (coerced to string)',
        () async {
      final client = SidecarClient(binaryPath: '/does/not/exist');
      final completer = Completer<Map<String, dynamic>>();
    ...
    test('malformed error envelope does not crash — synthesized -1 code',
        () async {
      ...
      final errA = await c1.future.then<Object?>((v) => v, onError: (Object e) => e);
      ...
      final errB = await c2.future.then<Object?>((v) => v, onError: (Object e) => e);
      ...
      final errC = await c3.future.then<Object?>((v) => v, onError: (Object e) => e);
