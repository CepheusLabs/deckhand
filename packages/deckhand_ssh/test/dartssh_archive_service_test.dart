...
void main() {
  group('DartsshArchiveService', () {
    ...
    test('captureRemote with no paths writes an empty archive', () async {
      ...
      final progress = await svc.captureRemote(
        session: _session,
        paths: const [],
        archivePath: out,
      ).toList();
    ...
    test('captureRemote streams chunked base64 and reassembles bytes',
        () async {
      ...
      final ssh = _FakeSsh(streamLines: {
      ...
      final progresses = await svc.captureRemote(
        session: _session,
    ...
    test('captureRemote deletes the partial archive on a malformed line',
        () async {
      final ssh = _FakeSsh(streamLines: {
        'tar -czf -': ['not-base64-!@#'],
      });
    ...
    test('captureRemote propagates SSH stream errors and cleans up',
        () async {
    ...
    test('restoreRemote uploads the archive once and runs tar -xzf '
      ...
      for (final cmd in ssh.runCommands) {
        expect(cmd.length, lessThan(2048),
            reason: 'restore command must not embed the archive: $cmd');
      }
    });

    test('restoreRemote reports archive missing without an SSH call',
        () async {
      final ssh = _FakeSsh();
      final svc = DartsshArchiveService(ssh: ssh);
      final res = await svc.restoreRemote(
...
List<int> _buildTarGz(Map<String, List<int>> files) {
  ...
  // ignore: prefer_const_constructors  -- 4.0.9's GZipEncoder is
  // not a const constructor; tests pin the runtime call to keep
  // the test resilient to future archive-package changes.
