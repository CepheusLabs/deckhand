...
| `disks.read_image` | Read a raw device to a local file with progress notifications. | `device_id` (optional string)<br>`path` (optional string)<br>`output` (optional string) | {sha256, output} |
...
| `os.download` | Download an OS image to dest, optionally verifying the expected SHA-256. | `url` (required string)<br>`dest` (required string)<br>`sha256` (optional string) | {sha256, path} |
